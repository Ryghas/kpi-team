...
export function GroupTable({ title, results, quarters }: Props) {
  ...
  return (
    <section className="mt-10" aria-labelledby={`g-${title}`}>
      <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
        ...
        <p className="text-sm text-muted">Rata-rata grup {fix(groupAvg)} work item ({ranked.length} engineer ranked)</p>
      </div>
            ...
            {[...ranked, ...others].map((r, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-3 py-3 text-right font-medium">{r.totalStoryPoints}</td>
                  <td className="whitespace-nowrap px-3 py-3 text-right">
                    <span className="font-medium">{r.totalWorkItems}</span>
                    <span className="block text-xs text-muted">{fix(r.ratio, 2)}x, {pct(r.donePct)} done</span>
                  </td>
                  <td className="px-2 py-3"><LevelCell level={r.levels.pcr} hint={`Rasio ${fix(r.ratio, 2)}x rata-rata grup`} /></td>
